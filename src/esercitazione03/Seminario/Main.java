...
public class Main {
  public static void main(String[] args) {
    ...
    Thread[] threadArray =
        startThreads(threadToCreate, extraValuesToAssign, numberOfValuesForThread);
    ...
    try (OutputStream os = new BufferedOutputStream(new FileOutputStream(outputFile))) {
      ...
      os.write(System.lineSeparator().getBytes());
      os.write(
          ("Tempo impiegato: " + seconds / 60 + " minuti e " + seconds % 60 + " secondi")
              .getBytes());
  ...
  private static Thread[] startThreads(
      int threadToCreate, int extraValuesToAssign, int numberOfValuesForThread) {
    int index = 0;
