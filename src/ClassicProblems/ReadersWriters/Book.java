...
public abstract class Book {
  ...
  public void test(int numWriters, int numReaders) {
    ...
    for (int i = 0; i < numReaders; i++) readers[i] = new Reader(this);

    Writer[] writers = new Writer[numWriters];
    for (int i = 0; i < numWriters; i++) writers[i] = new Writer(this);

    for (Writer writer : writers) new Thread(writer).start();

    for (Reader reader : readers) new Thread(reader).start();
  }
}
